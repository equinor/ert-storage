...
def post_ensemble(client):
    ...
    resp = client.post_check("/experiments", json={"name": "test_metadata"})
    exp_id = resp.json()["id"]
    resp = client.post_check(
        f"/experiments/{exp_id}/ensembles", json={"parameters": []}
    )
    ens_id = resp.json()["id"]
...
def test_metadata(client, base):
    ...
    client.put_check(f"{_url}/metadata", json=metadata_in)
    metadata_out = client.get_check(f"{_url}/metadata").json()
    assert metadata_in == metadata_out

    metadata_in_update = dict(msg_system=dict(error="NO_ERROR"))
    client.patch_check(f"{_url}/metadata", json=metadata_in_update)
